...
DATA_DIR = BOARD / "data" / "stock_learning"
CARDS_PATH = DATA_DIR / "cards.json"
PACK_PATH = DATA_DIR / "stock_knowledge_pack.json"
...
def add_card(
    *,
    ...
    card = {
        ...
        "source": source[:40],
        "status": "pending",
        "tags": ["주식시황", "웹리서치"] if "웹리서치" in title_in else ["주식시황"],
    }
    if seed:
...
def stats() -> dict:
    ...
    debate = sum(1 for c in cards if (c.get("category") or "") == "debate" or "웹리서치" in (c.get("title") or ""))
    return {
        "total": len(cards),
