(function (global) {
  const KEYS = { apiKey: "sf_gemini_key", shopDraft: "sf_shop_draft", lastVideoMeta: "sf_last_video_meta" };
  let lastVideoBlobUrl = null;

  function sanitizeFilename(name) {
    return String(name || "숏폼").replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, "_").slice(0, 40) || "숏폼";
  }
  ...
  function saveApiKey(value) {
    if (!value) return;
    try { localStorage.setItem(KEYS.apiKey, value); } catch (_) {}
  }
  ...
  function saveShopDraft(nameInput, conceptInput) {
    try {
      localStorage.setItem(KEYS.shopDraft, JSON.stringify({
  ...
  global.ShortsDeviceSave = {
    loadApiKey, saveApiKey, loadShopDraft, saveShopDraft,
    saveVideoToDevice, bindDownloadButton, getLastBlobUrl: () => lastVideoBlobUrl,
  };
})(window);
