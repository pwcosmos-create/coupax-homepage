"""
원키스 US 차수매매 학습 카드 — 한 계좌·슬롯 운용·HTS UI.

  python scripts/agent_office_workisus_learn.py add --title "..." --text "..."
    ...
    *,
    ...
    if target.get("status") == "confirmed" and reconfirm:
        ...
        try:
            ...
            agent_office_wiki_store.save_workisus_card_to_knowledge(target)
        except Exception:
            pass
    return target
    ...
    *,
    ...
    if title and title_taken(title):
        try:
            ...
            werr.record("duplicate", "동일 제목", title=title)
        except Exception:
            pass
    ...
    if len(body) < 30:
        try:
            ...
            werr.record("too_short", "본문 30자 미만", title=title or catalog_seed)
        except Exception:
            pass
        raise ValueError("학습 본문이 너무 짧습니다 (30자 이상).")
    ...
    try:
        ...
        agent_office_log.append_message(
            from_id="workisus_curator",
            kind="task",
            text=f"[원키스US 카드 #{next_id}] {title}\n{_summary(body, 120)}",
            division=DIVISION,
        )
    except Exception:
        pass
...
def confirm_card(card_id: int, *, export_pack_now: bool = True) -> dict | None:
    ...
    for c in store.get("cards") or []:
        if isinstance(c, dict) and c.get("id") == card_id:
            ...
            is_debate = (
                (c.get("source") or "") == "council_debate"
                or (c.get("category") or "") == "debate"
                or seed.startswith("debate_web_")
            )
            if not is_debate:
                wlp.require_card_production()
            c["status"] = "confirmed"
            ...
            try:
                ...
                wiki_row = agent_office_wiki_store.save_workisus_card_to_knowledge(c)
                if wiki_row:
                ...
                pass
            try:
                import workisus_atr_card_rl_engine as atr_rle

                if atr_rle._is_atr_card(c):
                    atr_rle.record_confirm_success(c)
            except Exception:
                pass
            try:
                import agent_office_log
...
def render_cursor_md(data: dict | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "stock.coupax.co.kr/workisus — 한 계좌·슬롯 차수·HTS UI 운용 카드.",
        "",
    ]
