...
OUTPUT_DIR = BOARD_DIR / "static" / "shorts_outputs"
    ...
    def _build_scene_clip(self, scene: dict, target_duration: float):
        ...
        if tts_path and tts_path.exists():
            try:
                ...
                print(f"TTS merge error: {e}")

        return base_clip, tts_path, media_path

    def assemble(self, bgm_b64: str, scenes: list, total_duration_seconds: int = 30) -> str:
        if not scenes:
        ...
        try:
            ...
            return f"/static/shorts_outputs/{output_filename}"
